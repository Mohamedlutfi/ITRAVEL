...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (changeForm) {
        changeForm.addEventListener('submit', async function (ev) {
            ev.preventDefault();
            const fd = new FormData(changeForm);
            const payload = { currentPassword: fd.get('currentPassword'), newPassword: fd.get('newPassword') };
            try {
                const res = await fetch('/change-password', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
                const j = await res.json();
                if (!res.ok) throw j;
                changeMsg.style.color = '#b3f7b3';
                changeMsg.textContent = 'Password changed successfully.';
                changeForm.reset();
            } catch (err) {
                changeMsg.style.color = '#ffb3b3';
                changeMsg.textContent = (err && err.error) || 'Failed to change password.';
            }
        });
    }
